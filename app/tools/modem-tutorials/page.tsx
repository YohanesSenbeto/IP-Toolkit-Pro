...
export default function ModemTutorialPage() {
    ...
    const filteredVideos = videos.filter(video => {
        const matchesModem = selectedModemModel === 'All' || 
            (video.modemModel && video.modemModel.toLowerCase().includes(selectedModemModel.toLowerCase()));
        
        const matchesLineType = selectedLineType === 'All' || 
            (video.connectionType && video.connectionType.toLowerCase() === selectedLineType.toLowerCase());
        
        const matchesDifficulty = selectedDifficulty === 'All' || 
            (video.difficulty && video.difficulty.toLowerCase() === selectedDifficulty.toLowerCase());
        
        const matchesSearch = searchQuery === '' || 
            video.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
            video.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
            (video.tags && video.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()))) ||
            (video.modemModel && video.modemModel.toLowerCase().includes(searchQuery.toLowerCase()));

        return matchesModem && matchesLineType && matchesDifficulty && matchesSearch;
    });
    ...
    const getLineTypeIcon = (lineType: string) => {
        return lineType === 'FIBER' ? <Wifi className="h-4 w-4" /> : <Cable className="h-4 w-4" />;
    };
    ...
    const modemModels = Array.from(new Set(
        videos.map(video => video.modemModel).filter(Boolean)
    )).sort();
        ...
        return (
            <div className="container mx-auto px-4 py-8">
                <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-semibold text-gray-600 mb-2">Failed to load videos</h3>
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            {/* Header */}
            <div className="mb-8">
                <h1 className="text-4xl font-bold mb-2 text-blue-800">Modem Configuration Tutorials</h1>
                <p className="text-lg text-gray-600">
                    Learn how to configure your modem/router for optimal internet connection. 
                    All tutorials are from Yoh-Tech Solutions YouTube channel.
                </p>
                ...
                <CardContent>
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setSelectedModemModel(e.target.value)}
                    ...
                    <div className="mt-4">
                        <p className="text-sm text-gray-600">
                            Showing {filteredVideos.length} of {videos.length} tutorial{filteredVideos.length !== 1 ? 's' : ''}
                        </p>
                    </div>
                </CardContent>
            </Card>
                        ...
                        >
                            ...
                            <CardContent className="p-4">
                                <div className="space-y-3">
                                    ...
                                    {/* Modem Model */}
                                    {video.modemModel && video.modemModel !== 'Unknown' && (
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-gray-500">
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {video.likeCount > 0 ? formatViewCount(video.likeCount) : 'N/A'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <Badge 
                                            className={`text-xs px-2 py-1 ${getDifficultyColor(difficulty)}`}
                                        >
                                            {difficulty}
                                        </Badge>
                                        <Button
            ...
            {filteredVideos.length === 0 && !loading && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-4">
                        Try adjusting your filters or search terms to find relevant tutorials.
                    </p>
                    <Button 
                        variant="outline"
                        onClick={() => {
